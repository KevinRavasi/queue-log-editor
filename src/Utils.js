...
export function generateQueueLogRecord(action, data, writeToQueueLog) {
    ...
    switch (action) {
        case "ADDMEMBER":
            writeToQueueLog(
                now + "|MANAGER|" + data.queue + "|" + data.agentExtension + "|ADDMEMBER|"
            );
            break;

        case "REMOVEMEMBER":
            writeToQueueLog(
                now + "|MANAGER|" + data.queue + "|" + data.agentExtension + "|REMOVEMEMBER|"
            );
            break;
        ...
        case "CONNECT":
            const connectHoldTime =
                now - localStorage.getItem(data.callUniqueId + "-enterqueueTimestamp");
            writeToQueueLog(
                now +
                    "|" +
                    data.callUniqueId +
        ...
        case "COMPLETEAGENT":
            ...
            const completeAgentOriginalPosition = localStorage.getItem(
                data.callUniqueId + "-position"
            );
        ...
        case "COMPLETECALLER":
            ...
            const completeCallerCallTime =
                now - localStorage.getItem(data.callUniqueId + "-connectTimestamp");
            const completeCallerOriginalPosition = localStorage.getItem(
                data.callUniqueId + "-position"
            );
